...
cur.execute(f"""
            select * from (
            SELECT projects.title, projects.description, array_agg(users.username) as users, projects.id
            FROM projects 
            JOIN userprojects ON projects.id = userprojects.projectid 
            JOIN users ON userprojects.username = users.username
            GROUP BY projects.id, projects.title, projects.description) a
            WHERE '{username}' = ANY(ARRAY[users]);;
        """)
...
if projects:
    ...
    for project in projects:
        st.subheader(f"Title: {project[0]}")
        ...
        if st.session_state['role'] == 'advisor':
            ...
            with col1:
                new_username = st.text_input(f"Add user to project '{project[0]}':", key=f'userinput{project[3]}')
            ...
            with col2:
                # remove user
                remove_username = st.text_input(f"Remove user from project '{project[0]}':", key=f'userinput_remove{project[3]}')
                if st.button(f"Remove User", key=f'addbutton_remove{project[3]}'):
                    if remove_username:
                        cur.execute("DELETE FROM userprojects WHERE username = %s AND projectid = %s;", (remove_username, project[3]))
                        connection.commit()
                        st.success(f"User '{remove_username}' removed from project '{project[0]}'.")
                        st.rerun()
                    else:
                        st.warning("Please enter a username to remove.")
        st.write("---")
else:
